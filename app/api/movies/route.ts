 ...
 *
 * Сверх перечисленного в разделе 4 добавлены fileStatus, диск с весом,
 * quality и прогресс по сериалу: без них не собрать полку «Смотрю сейчас»
 * и «Буду смотреть, уже скачано» (5.2) и не отфильтровать по файлу, диску
 * и качеству (5.4) — а отбор идёт целиком в памяти клиента.
 *
 * Там же cast: поиск по актёрам из 5.5 обязан работать без обращения
 * к серверу, а значит имена должны приехать заранее. Это единственное
 * заметное утяжеление — три-четыре имени на запись.
 *
 * И episodes: часы по сериалу это минуты в серии на число серий (5.10).
 */
const lightColumns = {
  id: movies.id,
  kind: movies.kind,
